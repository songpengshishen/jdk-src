...
public class HashMap<K,V> extends AbstractMap<K,V>
     ...
     *
     * 当负载因子太大,扩容几率变低,但内存利用率会比较高,但是访问效率会下降，因为大量元素可能在一个桶中,如果不经常遍历Map可以调高负载因子,减少内存占用.典型的读少写多场景使用.
     * 当负载因子太小,扩容几率变高,但内存利用率会比较低,但是元素会更加均匀的分布在各个桶中,链表就小了,所以访问效率会提升,如果经常变量Map,可以降低负载因子,让元素更均匀.典型的读多写少场景使用
     * 扩容几率低,写的效率就高了,写不需要经常扩容.
